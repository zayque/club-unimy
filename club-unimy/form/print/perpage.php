<?php
	function perpage($count, $per_page = '10',$href) {
		...
		if($pages>1) {
			...
			if(($_POST["page"]-3)>0) {
				if($_POST["page"] == 1)
					$output = $output . '<span id_user=1 class="current-page">1</span>';
				else				
			...
			for($i=($_POST["page"]-2); $i<=($_POST["page"]+2); $i++)	{
				...
				if($i>$pages) break;
				if($_POST["page"] == $i)
					$output = $output . '<span id_user='.$i.' class="current-page" >'.$i.'</span>';
				else				
			...
			if(($pages-($_POST["page"]+2))>0) {
				if($_POST["page"] == $pages)
					$output = $output . '<span id_user=' . ($pages) .' class="current-page">' . ($pages) .'</span>';
				else				
